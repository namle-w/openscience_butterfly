...
class PoisonAgent():
    ...
    def choose_poisons_randomly(self):
        ...
        x_train_np, y_train_np = _dataset_to_numpy(self.trainset, size=self.args.size)
        x_test_np, y_test_np = _dataset_to_numpy(self.validset, size=self.args.size)
        x_memory_np, y_memory_np = _dataset_to_numpy(self.memory_loader.dataset, size=self.args.size)

        # To tensors (N,H,W,C) -> (N,C,H,W)
        x_train_tensor = torch.tensor(x_train_np).permute(0, 3, 1, 2)
        x_test_tensor = torch.tensor(x_test_np).permute(0, 3, 1, 2)
        x_memory_tensor = torch.tensor(x_memory_np).permute(0, 3, 1, 2)
        ...
        assert x_train_tensor.size(0) == y_train_tensor.size(0), "train X/Y length mismatch"
        assert x_test_tensor.size(0) == y_test_tensor.size(0), "test X/Y length mismatch"
        assert x_memory_tensor.size(0) == y_memory_tensor.size(0), "memory X/Y length mismatch"

        # --- Dùng kích thước tensor để tạo index (tránh lệch với dataset gốc) ---
        train_index = torch.arange(x_train_tensor.size(0), dtype=torch.long)
        test_index = torch.arange(x_test_tensor.size(0), dtype=torch.long)
        memory_index = torch.arange(x_memory_tensor.size(0), dtype=torch.long)
        ...
        # Với test/val, KHÔNG nên drop_last để khỏi rớt mẫu
...
# --- Helper: extract images/labels from various torchvision datasets ---
def _dataset_to_numpy(ds, size=None):
    import numpy as np
    from torchvision.datasets import GTSRB
    from PIL import Image

    # Fast path: CIFAR-style
    ...
    # GTSRB: reload split gốc, KHÔNG transform, rồi resize về (size,size)
    if isinstance(ds, GTSRB) or ds.__class__.__name__ == "GTSRB":
        raw = GTSRB(root=ds.root, split=getattr(ds, "split", "train"), transform=None, download=False)
        xs, ys = [], []
        for img, t in raw:  # <-- lấy nhãn trực tiếp từ iterator, KHÔNG dùng raw.targets
            if size is not None:
    ...
    for i in range(len(ds)):
        img, t = ds[i]
        if hasattr(img, "numpy"):  # tensor CxHxW
            ...
            if size is not None and (arr.shape[0] != size or arr.shape[1] != size):
                pil = Image.fromarray(np.clip(arr * 255.0, 0, 255).astype(np.uint8)).convert("RGB").resize((size, size), Image.BILINEAR)
...
def set_aug_diff(args):
    ...
    if 'cifar' in args.dataset   or args.dataset == 'imagenet-100' or args.dataset == 'gtsrb':
    ...
    if args.dataset == 'cifar10':
        ...
        from torchvision.datasets import GTSRB
        train_dataset = GTSRB(root=args.data_path,
                              split='train',
                              transform=transform_load,
                              download=True)

        ft_dataset = GTSRB(root=args.data_path,
                           split='train',
                           transform=transform_load,
                           download=False)

        test_dataset = GTSRB(root=args.data_path,
                             split='test',
                             transform=transform_load,
                             download=True)

        memory_dataset = GTSRB(root=args.data_path,
                               split='train',
                               transform=transform_load,
                               download=False)
